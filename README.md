...
[![Version](https://img.shields.io/badge/version-1.3.2-blue)](metadata.txt)
...
- **No BD TOPO reach at all is a harder failure, not just an unconfirmed one.** `upstream_extent` needs a reach within the snap radius just to size the DEM download — without one it refuses outright (`NoNetworkNearbyError`), before any terrain is even fetched. On refusal the run offers a small-basin fallback: a fixed buffer around the clicked point stands in for the network-derived extent, and the outlet is snapped on the terrain model alone. It loses two things the normal path has: the BD TOPO-length filter that rejects a small, wrong channel during snapping, and any consistency check afterwards, since there is no upstream network left to compare the basin against.
